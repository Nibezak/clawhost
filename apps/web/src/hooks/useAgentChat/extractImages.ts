...
const extractImages = (content: unknown): ChatImageSource[] => {
    ...
    return content
        .filter((block) => block.type === 'image' && (block.source || block.url || block.data))
        .map((block) => {
            if (block.url) {
                return {
                    type: 'url',
                    mediaType: block.media_type || block.mediaType || block.mimeType || 'image/png',
            ...
            if (!block.source && block.data) {
                return {
                    type: 'base64',
                    mediaType: block.mimeType || block.media_type || block.mediaType || 'image/png',
