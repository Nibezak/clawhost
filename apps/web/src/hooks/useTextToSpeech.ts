...
const useTextToSpeech = (): UseTextToSpeechReturn => {
    ...
    const playCachedAudio = useCallback((messageId: string, buffer: AudioBuffer) => {
        const ctx = getAudioContext()

        if (outputDeviceIdRef.current && 'setSinkId' in ctx) {
            // @ts-ignore
            ctx.setSinkId(outputDeviceIdRef.current)
        }
    ...
    const playStreamingAudio = useCallback(async (
        messageId: string,
        response: Response
    ) => {
        ...
        const scheduleChunk = (pcmBytes: Uint8Array) => {
            ...
            const audioBuffer = ctx.createBuffer(channels, samplesPerChannel, sampleRate)
        ...
        try {
            ...
            const cachedBuffer = new (window.OfflineAudioContext || window.AudioContext)(
                channels, samplesPerChannel || 1, sampleRate
            ).createBuffer(channels, samplesPerChannel || 1, sampleRate)
            if (channels === 1) {
