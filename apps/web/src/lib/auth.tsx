...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [cachedProfile, setCachedProfile] = useState<CachedProfile | null>(readCachedProfile)
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        try {
          ...
          const fresh: CachedProfile = { email: profile.email, name: profile.name }
        ...
        queryClient.prefetchQuery({ queryKey: ['claws'], queryFn: () => api.getClaws() })
        queryClient.prefetchQuery({ queryKey: ['userStats'], queryFn: api.getUserStats })
      } else {
  ...
  return (
    <AuthContext.Provider value={{ user, loading, cachedProfile, updateCachedProfile, sendOtp, verifyOtp, signOut }}>
      {children}
