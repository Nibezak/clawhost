...
const CreateClawModal: FC<CreateClawModalProps> = ({
    ...
    const autoProvider: ProviderType = preselectedProvider && !isProviderUnavailable(preselectedProvider)
        ? preselectedProvider
        : hetznerPlans?.length ? clawProvider.hetzner
            : digitaloceanPlans?.length ? clawProvider.digitalocean
                : vultrPlans?.length ? clawProvider.vultr
                    : clawProvider.hetzner

    const [userSelectedProvider, setUserSelectedProvider] = useState<ProviderType | null>(null)

    const provider = userSelectedProvider && !isProviderUnavailable(userSelectedProvider)
                    ...
                    <div className='space-y-1'>
                        ...
                        <TooltipProvider delayDuration={200}>
                            <div className='bg-muted flex w-fit rounded-lg p-1'>
                                ...
                                ] as { key: ProviderType; label: string; icon: ReactNode }[]).map((p) => {
