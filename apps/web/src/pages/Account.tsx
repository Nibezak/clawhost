...
import { Input, Label, Avatar, AvatarFallback, Button, Checkbox, Tooltip, TooltipTrigger, TooltipContent } from '@/components/ui'
import {
    Header,
    LandingFooter,
    PageBackground,
...
const Account: FC = (): ReactNode => {
    ...
    const [linkingProvider, setLinkingProvider] = useState<AuthMethod | null>(null)
    const [unlinkingProvider, setUnlinkingProvider] = useState<AuthMethod | null>(null)
    ...
    const handleLinkProvider = useCallback(async (provider: 'google' | 'github') => {
        ...
        try {
            ...
            showToast(
                t('account.providerConnected', {
                    provider: provider === 'google' ? t('account.authGoogle') : t('account.authGithub')
                }),
                'success'
            )
        } catch (err: unknown) {
    ...
    const handleUnlinkProvider = useCallback(async (provider: 'google' | 'github') => {
        if (providerBusy) return
                        ...
                        <div className='mt-6 rounded-xl border border-white/10 bg-white/5 p-8 backdrop-blur-sm'>
                            ...
                            <div className='space-y-3'>
                                <div className='flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.02] px-4 py-3'>
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <button
                                                disabled
                                                className='flex items-center gap-1.5 rounded-md border border-white/10 px-3 py-1 text-xs text-white/50 opacity-50'
                                            >
                                ...
                                <div className='flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.02] px-4 py-3'>
                                    ...
                                    {profile?.authMethods?.includes('google') ? (
                                        <button
                                ...
                                <div className='flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.02] px-4 py-3'>
                                    ...
                                    {profile?.authMethods?.includes('github') ? (
                                        <button
