import type { AuthenticatedContext } from '@/ts/Types'

import executeSSH from '@/services/ssh'
import { findUserClaw } from '@/controllers/claws/helpers'
import { t } from '@openclaw/i18n'
import { ok, fail } from '@/lib/response'

const PAIR_LOG = '/tmp/openclaw-wa-pair.log'
const PAIR_PID = '/tmp/openclaw-wa-pair.pid'
const CREDS_DIR = '/home/openclaw/.openclaw/credentials/whatsapp'

const pairWhatsAppStatus = async (c: AuthenticatedContext) => {
    try {
        const userId = c.get('userId')
        const id = c.req.param('id')
        const claw = await findUserClaw(userId, id)

        if (!claw) {
            return fail(c, t('api.clawNotFound'), 404)
        }

        if (!claw.ip || !claw.rootPassword) {
            return fail(c, t('api.whatsappPairFailed'), 400)
        }

        try {
            const output = await executeSSH(
                claw.ip,
                claw.rootPassword,
                [
                    `ls ${CREDS_DIR}/*/creds.json 2>/dev/null && echo "STATUS:CREDS_FOUND" || echo "STATUS:NO_CREDS"`,
                    `echo "===SEPARATOR==="`,
                    `cat ${PAIR_LOG} 2>/dev/null || echo "STATUS:NO_LOG"`,
                    `echo "===SEPARATOR==="`,
                    `test -f ${PAIR_PID} && kill -0 $(cat ${PAIR_PID}) 2>/dev/null && echo "STATUS:RUNNING" || echo "STATUS:NOT_RUNNING"`,
                    `echo "===SEPARATOR==="`,
                    `test -f ${PAIR_LOG} && echo "STATUS:LOG_EXISTS" || echo "STATUS:NO_LOG_FILE"`
                ].join('; '),
                8000
            )

            const sections = output
                .split('===SEPARATOR===')
                .map((s) => s.trim())
            const credsSection = sections[0] || ''
            const logSection = sections[1] || ''
            const pidSection = sections[2] || ''
            const logFileSection = sections[3] || ''

            if (credsSection.includes('STATUS:CREDS_FOUND')) {
                await executeSSH(
                    claw.ip,
                    claw.rootPassword,
                    `kill $(cat ${PAIR_PID} 2>/dev/null) 2>/dev/null; rm -f ${PAIR_LOG} ${PAIR_PID}`,
                    5000
                ).catch(() => {})
                return ok(c, { status: 'paired' })
            }

            const isRunning = pidSection.includes('STATUS:RUNNING')
            const logExists = logFileSection.includes('STATUS:LOG_EXISTS')
            const hasLogContent =
                !logSection.includes('STATUS:NO_LOG') && logSection.length > 0

            if (!logExists) {
                return ok(c, { status: 'waiting' })
            }

            if (hasLogContent) {
                const logLines = logSection.split('\n')
                const qrLines = logLines.filter((line) =>
                    /[\u2580\u2584\u2588\u258C\u2590\u2591\u2592\u2593\u25A0\u25A1\u25AA\u25AB]/.test(
                        line
                    )
                )

                if (qrLines.length > 5) {
                    return ok(c, { status: 'qr_ready', qr: qrLines.join('\n') })
                }
            }

            if (!isRunning && hasLogContent) {
                return ok(c, {
                    status: 'failed',
                    log: logSection.slice(0, 500)
                })
            }

            return ok(c, { status: 'waiting' })
        } catch {
            return fail(c, t('api.whatsappPairFailed'), 500)
        }
    } catch {
        return fail(c, t('api.whatsappPairFailed'), 500)
    }
}

export default pairWhatsAppStatus